...
local function LoadGridsquare(square, checkResult, level)
    ...
    if level ~= 0 and checkResult.hasRoof then
        percentage = WDecay_Bushes.getBasePercentageOnRoof()
    ...
    local chance = WDecay_Placement.clusterChance(square, "bush", WDecay_Scaling.scaleFor('nature', percentage), 4)
...
WDecay_PlacementGeneratorFeatures[#WDecay_PlacementGenerators] = "bushes"
